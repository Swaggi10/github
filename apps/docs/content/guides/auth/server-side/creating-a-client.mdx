...
Now that you have the Supabase client set up, you can implement OAuth authentication. This example demonstrates how to sign in users with OAuth providers like Google, GitHub, and Azure.

#### Configure OAuth providers

Before implementing OAuth, enable and configure your OAuth providers in the Supabase Dashboard:

1. Go to your project's [Authentication settings](https://supabase.com/dashboard/project/_/auth/providers)
2. Enable the OAuth providers you want to use (Google, GitHub, Azure, etc.)
3. Configure each provider with the required credentials (Client ID, Client Secret)
4. Add your callback URL: `http://localhost:5173/auth/callback` for development
...
Create a server endpoint to handle the OAuth callback and exchange the code for a session:
...
Use `safeGetSession()` to protect routes and access user data:
...
Create a server endpoint to handle sign out:

```ts src/routes/auth/signout/+server.ts
import { redirect } from '@sveltejs/kit'
import type { RequestHandler } from './$types'
